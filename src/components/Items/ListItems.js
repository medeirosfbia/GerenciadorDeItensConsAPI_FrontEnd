...
const ListItems = () => {
  ...
  // Para pegar o id da categoria da URL, se existir
  ...
  useEffect(() => {
    const fetchAllItems = async () => {
      try {
        const res = await axios.get("http://localhost:8082/items");
        setItems(res.data);
      } catch (err) {
  ...
  // Função utilitária para pegar o nome da categoria pelo id
  const getCategoryName = (categoryId) => {
    const cat = categories.find(c => c.id === categoryId);
    return cat ? cat.nome || cat.name : categoryId;
  };

  // Filtragem dos itens conforme categoria selecionada na URL
  const filteredItems = categoryIdParam
    ? items.filter(item => String(item.fk_category) === String(categoryIdParam))
    : items;
